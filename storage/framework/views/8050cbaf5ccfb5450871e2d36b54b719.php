    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6 mb-6">
                <form method="GET" action="<?php echo e(route('activities.index')); ?>" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                        ...
                        <select name="action" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            <optgroup label="Grade Management">
                                <option value="grade_entered" <?php echo e(request('action') == 'grade_entered' ? 'selected' : ''); ?>>Grade Entered</option>
                                <option value="grade_modified" <?php echo e(request('action') == 'grade_modified' ? 'selected' : ''); ?>>Grade Modified</option>
                                <option value="bulk_grade_update" <?php echo e(request('action') == 'bulk_grade_update' ? 'selected' : ''); ?>>Bulk Grade Update</option>
                                <option value="grade_batch_submitted" <?php echo e(request('action') == 'grade_batch_submitted' ? 'selected' : ''); ?>>Batch Submitted</option>
                                <option value="grade_batch_approved" <?php echo e(request('action') == 'grade_batch_approved' ? 'selected' : ''); ?>>Batch Approved</option>
                                <option value="grade_batch_rejected" <?php echo e(request('action') == 'grade_batch_rejected' ? 'selected' : ''); ?>>Batch Rejected</option>
                            </optgroup>
                        </select>
                    </div>
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $activities; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $activity): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="flex gap-4 pb-6 mb-6 border-b border-gray-200 last:border-0">
                            ...
                            <div class="flex-shrink-0">
                                ...
                                <?php elseif(str_contains($activity->action, 'grade')): ?>
                            ...
                            <div class="flex-1">
                                <div class="flex items-start justify-between">
                                    <div>
                                        <p class="text-sm font-semibold text-gray-900"><?php echo e($activity->description); ?></p>
                                        <p class="text-xs text-gray-500 mt-1">
                                            by <span class="font-medium"><?php echo e($activity->user->name); ?></span>
                                            · <?php echo e($activity->created_at->diffForHumans()); ?>

                                            · <?php echo e($activity->created_at->format('M d, Y h:i A')); ?>

                                        </p>
                                    </div>
                                    <span class="px-3 py-1 text-xs font-semibold rounded-full
                                        <?php if($activity->action === 'created'): ?> bg-green-100 text-green-800
                                        <?php elseif($activity->action === 'updated'): ?> bg-blue-100 text-blue-800
                                        <?php elseif($activity->action === 'deleted'): ?> bg-red-100 text-red-800
                                        <?php elseif(str_contains($activity->action, 'approved')): ?> bg-emerald-100 text-emerald-800
                                        <?php elseif(str_contains($activity->action, 'rejected')): ?> bg-red-100 text-red-800
                                        <?php elseif(str_contains($activity->action, 'grade')): ?> bg-purple-100 text-purple-800
                                ...
                                <?php if($activity->properties && isset($activity->properties['old']) && isset($activity->properties['new'])): ?>
                                    <details class="mt-2">
                                        ...
                                        <div class="mt-2 bg-gray-50 rounded-lg p-3 text-xs">
                                            <div class="grid grid-cols-2 gap-2">
                                                <div>
                                                    ...
                                                    <pre class="text-gray-600 overflow-auto"><?php echo e(json_encode($activity->properties['old'], JSON_PRETTY_PRINT)); ?></pre>
                                                </div>
                                                <div>
                                                    <p class="font-semibold text-gray-700 mb-1">After:</p>
                                                    <pre class="text-gray-600 overflow-auto"><?php echo e(json_encode($activity->properties['new'], JSON_PRETTY_PRINT)); ?></pre>
                                                </div>
                                            </div>
                                        </div>
                                    </details>
...
<?php endif; ?>
<?php /**PATH /opt/lampp/htdocs/bsu-sims/resources/views/activities/index.blade.php ENDPATH**/ ?>